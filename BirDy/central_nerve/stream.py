...
class BrightQueue:
    ...
    def start(self) -> None:
        ...
        print("[BrightOS] ✅ Pipeline initialized")

    def stop(self) -> None:
        self._running = False
        with self._condition:
            self._condition.notify_all()
        print("[BrightOS] 🔴 Pipeline deactivated")
    ...
    def submit(
        ...
        print(f"[BrightOS] 📥 Objective added: [{task_id}] {goal[:60]}")
        return task_id
    ...
    def cancel(self, task_id: str) -> bool:
        ...
        with self._lock:
            ...
            print(f"[BrightOS] 🚫 Operation terminated: [{task_id}]")
            return True
    ...
    def _run_task(self, task: BrightTask) -> None:
        print(f"[BrightOS] ▶️ Deploying: [{task.task_id}] {task.goal[:60]}")
        try:
            ...
            if task.on_complete and not task.cancel_flag.is_set():
                try:
                    ...
                    print(f"[BrightOS] ⚠️ Post-execution callback failure: {e}")

            print(f"[BrightOS] ✅ objective finalized: [{task.task_id}]")
            ...
            with self._lock:
                ...
                task.error  = str(e)
                self._active_count -= 1
            print(f"[BrightOS] ❌ Critical failure: [{task.task_id}] {e}")
