            ...
            for hive in [winreg.HKEY_LOCAL_MACHINE, winreg.HKEY_CURRENT_USER]:
                try:
                    ...
                    if exe and Path(exe).exists():
                        logger.info(f"Opera path found via registry: {exe}")
                        return exe
                except Exception:
                    continue
    except Exception:
...
def _find_browser_executable(prog_id: str) -> tuple:
    ...
    for browser_name, patterns in browser_patterns.items():
        ...
        for binary in binaries:
            path = shutil.which(binary)
            if path:
                logger.info(f"Found {browser_name} binary at: {path}")
                return "chromium", path, None
...
class _BrowserThread:
    ...
    async def _launch(self):
        # Force Chrome as requested by user
        prog_id                        = "chrome"
    ...
    def native_open(self, url: str) -> str:
        """Opens URL in system default browser (Total Stealth)."""
        if not url.startswith("http"):
            url = "https://" + url
        print(f"[Browser] 🌐 Native Open: {url}")
        webbrowser.open(url)
        return f"Opened {url} in native browser."
...
def browser_control(
    ...
    try:
        ...
        elif action == "native_open" or action == "open":
            result = _bt.native_open(parameters.get("url", parameters.get("query", "")))
    ...
    logger.info(f"Browser Control Result: {result[:120]}")
    if player:
