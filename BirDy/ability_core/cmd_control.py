...
def _run_visible(command: str) -> None:
    try:
        ...
        logger.warning(f"Terminal allocation failed: {e}")
...
def cmd_control(
    ...
    if not command:
        ...
        if command:
            logger.info(f"Triggering hardcoded logic: {command[:80]}")
        else:
            logger.info(f"Synthesizing command via Gemini for: {task}")
            command = _ask_gemini(task)
            logger.info(f"Command synthesized: {command[:80]}")
            if command == "UNSAFE":
